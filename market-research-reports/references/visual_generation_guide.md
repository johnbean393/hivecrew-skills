...
| Workflows, ecosystems, market maps | Mermaid | Editable vector diagrams that fit the manual XeLaTeX PDF workflow |
| Infographics | generate-image | Creative visual synthesis |
| Conceptual illustrations | generate-image | Abstract concepts |

---

## Mermaid and PDF Embedding Workflow

Use the same report-diagram workflow as `research-report` when Mermaid is the best tool for clarity:

1. Draft Mermaid source for workflows, value chains, market maps, ecosystem diagrams, and framework overviews.
2. Render Mermaid to **SVG** first.
3. Convert SVG to **PDF** for LaTeX embedding when direct SVG inclusion is unreliable.
4. Embed vector output with `\includegraphics` so framework visuals stay crisp in the final PDF.
5. Use generated infographics sparingly and only when they materially improve comprehension.

For additional guidance on Mermaid best practices, generated infographic style, visual mix, and PDF embedding, see `references/visual-elements.md`.
